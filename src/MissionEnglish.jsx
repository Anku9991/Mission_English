...
export default function MissionEnglish() {
  ...
  const [aiForm, setAiForm] = useState({ qCount: 10, price: 99 });
  ...
  const generateQuestionsWithAI = async () => {
    ...
    try {
      const prompt = `You are Ankush AI, a master English teacher. Based on the provided content (text or image), generate ${aiForm.qCount} Multiple Choice Questions (MCQs) for learning English. 
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                    <div>
                      ...
                      <input 
                        ...
                        value={aiForm.qCount}
                        onChange={(e) => setAiForm({...aiForm, qCount: parseInt(e.target.value) || 5})}
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2 uppercase tracking-wide">Course Price (₹)</label>
                      <input 
                        type="number" 
                        ...
                        value={aiForm.price}
                        onChange={(e) => setAiForm({...aiForm, price: parseInt(e.target.value) || 0})}
                      />
                    </div>
                  </div>
