  ...
  if (!hasAccess) {
    return (
      <div className="text-center py-20 max-w-md mx-auto">
        ...
        <p className="text-muted-foreground mb-8">You need to enroll in this course to view its content.</p>
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">{course.type === "notes" ? "PDF Notes Viewer" : "Video Course Player"}</p>
          </div>
        </div>
      </div>

      {course.type === "notes" ? (
        <div className="bg-secondary/50 border border-border rounded-3xl overflow-hidden shadow-sm flex flex-col min-h-[60vh]">
          <div className="bg-card border-b border-border p-6 flex items-center gap-3">
            <BookOpen className="w-6 h-6 text-blue-500" />
            <h2 className="font-bold text-foreground text-xl">{course.title} Notes</h2>
          </div>
          <div className="p-6 md:p-10 w-full bg-card flex-1">
            <div className="prose max-w-none text-foreground whitespace-pre-wrap leading-relaxed text-base">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-card border border-border rounded-3xl overflow-hidden sticky top-6">
              <div className="bg-secondary/50 border-b border-border p-5">
                <h3 className="font-bold text-foreground text-lg">Curriculum</h3>
                <p className="text-xs text-muted-foreground mt-1">{course.modules?.length || 0} Modules</p>
              </div>
              <div className="max-h-[60vh] overflow-y-auto">
                {course.modules?.map((m, mIdx) => (
                  <div key={m.id} className="border-b border-border last:border-0">
                    <div className="bg-secondary/50/50 px-5 py-3 font-semibold text-foreground text-sm">
                    ...
                    <div className="divide-y divide-slate-50">
                      {m.lessons.map((l, lIdx) => (
                        <button
                          ...
                          className={`w-full text-left px-5 py-4 flex items-start gap-3 transition-colors ${activeLesson === l.url ? "bg-blue-50/50" : "hover:bg-secondary/50"}`}
                        >
                          <PlayCircle className={`w-4 h-4 mt-0.5 shrink-0 ${activeLesson === l.url ? "text-blue-500" : "text-muted-foreground"}`} />
                          <div>
                            <p className={`text-sm font-medium ${activeLesson === l.url ? "text-blue-700" : "text-foreground"}`}>
                              {lIdx + 1}. {l.title}
                            </p>
                            {l.duration && <span className="text-xs text-muted-foreground mt-1 block">{l.duration}</span>}
                          </div>
                        </button>
                      ))}
                    </div>
