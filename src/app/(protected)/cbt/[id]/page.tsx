...
export default function CBTTestPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <div className="max-w-3xl mx-auto pt-10">
        <div className="premium-card p-10 text-center">
          ...
          <h1 className="text-3xl font-black text-foreground mb-4">{course.title}</h1>
          ...
          <div className="flex justify-center gap-10 mb-10 bg-secondary/50 p-6 rounded-2xl border border-border max-w-md mx-auto">
            <div>
              <div className="text-sm font-bold text-muted-foreground uppercase tracking-wider mb-1">Questions</div>
              <div className="text-2xl font-black text-foreground">{course.questions.length}</div>
            </div>
            <div className="w-px bg-slate-200" />
            <div>
              <div className="text-sm font-bold text-muted-foreground uppercase tracking-wider mb-1">Duration</div>
              <div className="text-2xl font-black text-foreground">{formatTime(timeLeft)}</div>
            </div>
          </div>
  ...
  return (
    <div className="min-h-screen bg-secondary/50 pb-20 select-none">
      
      {/* Warning Modal */}
      {showWarningModal && (
        <div className="fixed inset-0 z-[100] bg-foreground/80 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-card rounded-3xl p-8 max-w-md w-full shadow-2xl text-center border-2 border-red-500 animate-in zoom-in-95">
            <div className="w-20 h-20 bg-red-100 rounded-full flex items-center justify-center mx-auto mb-6">
              <AlertTriangle className="w-10 h-10 text-red-600" />
            </div>
            <h2 className="text-2xl font-black text-foreground mb-2">Tab Switch Detected!</h2>
      ...
      <div className="sticky top-0 z-50 bg-card border-b border-border shadow-sm">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <h1 className="font-bold text-foreground truncate pr-4">{course.title}</h1>
          <div className="flex items-center gap-4 sm:gap-6 shrink-0">
            ...
            <div className={`flex items-center gap-2 font-mono text-xl font-bold ${timeLeft < 300 ? 'text-red-600 animate-pulse' : 'text-foreground'}`}>
      ...
      <div className="container mx-auto px-4 pt-8 grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                return (
                  ...
                  >
                    <span className={`w-8 h-8 shrink-0 rounded-lg flex items-center justify-center font-bold text-sm transition-colors ${
                      isSelected ? "bg-blue-500 text-white" : "bg-secondary text-muted-foreground group-hover:bg-blue-100 group-hover:text-blue-600"
                    }`}>
                      {opt}
                    </span>
                    <span className={`text-base font-medium ${isSelected ? "text-foreground" : "text-foreground"}`}>
          ...
          <div className="flex flex-wrap justify-between items-center mt-6 gap-4">
            <div className="flex gap-2">
              <Button 
                ...
                className="rounded-xl px-5 h-11 bg-card border-border hover:bg-secondary/50 font-bold gap-2"
              >
              ...
              <Button 
                ...
                className="rounded-xl px-4 h-11 bg-card border-border hover:bg-red-50 hover:text-red-600 font-bold"
              >
                Clear
              </Button>
            </div>
            ...
            <div className="flex gap-2">
              <Button 
                ...
                className={`rounded-xl px-4 h-11 font-bold ${reviewMarks[currentIdx] ? 'bg-purple-100 text-purple-700 border-purple-200 hover:bg-purple-200' : 'bg-card border-border hover:bg-purple-50 hover:text-purple-600'}`}
              >
        ...
        <div className="lg:col-span-1">
          <div className="premium-card p-6 sticky top-24">
                ...
                return (
                  <button
                    ...
                    className={`aspect-square rounded-lg flex items-center justify-center text-sm font-bold border-2 transition-all ${
                      isCurrent ? "border-blue-500 bg-card text-blue-600 shadow-md scale-110 z-10" :
                      isReview ? "bg-purple-100 border-purple-300 text-purple-700" :
                      isAnswered ? "bg-emerald-100 border-emerald-200 text-emerald-700" :
                      "bg-secondary/50 border-border text-muted-foreground hover:border-slate-300"
                    }`}
                  >
                    {i + 1}
                  </button>
            ...
            <div className="space-y-3 pt-6 border-t border-border text-sm font-medium">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-muted-foreground">Marked Review ({Object.values(reviewMarks).filter(Boolean).length})</span>
              </div>
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 rounded bg-secondary/50 border border-border" />
                <span className="text-muted-foreground">Not Visited ({course.questions.length - Object.keys(answers).length})</span>
              </div>
            </div>
          </div>
        </div>
